...
// Compress compresses the given byte array and returns the compressed result.
...
func writeZero(writer io.Writer, size int) {
	...
	for remain > 0 {
		if remain < 0x80 {
			...
			writeExtended(writer, uint16(len))
			remain -= len
		}
	}
}
...
func writeConstant(writer io.Writer, size int, value byte) {
	...
	for start < size {
		...
		if remain < 0x100 {
			...
			writeExtended(writer, 0xC000+uint16(len), value)
			start += len
		}
	}
}
...
func writeRaw(writer io.Writer, data []byte) {
	...
	for start < end {
		...
		if remain < 0x80 {
			...
			writeExtended(writer, 0x8000+uint16(len), data[start:start+len]...)
			start += len
		}
	}
}
